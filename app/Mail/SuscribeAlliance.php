...
class SuscribeAlliance extends Mailable
{
     ...
     * @return void
     */
    public function __construct($dataMail, $dataUsers, $paso_titulo, $dataAlianza, $archivosAdjuntos, $CoordinadorInterno, $CoordinadorExterno)
    {
    ...
    public function build()
    {
        ...
        //$toData = json_decode($this->dataMail[0]->to);
        $ccData = json_decode($this->dataMail[0]->cc);
        $bccData = json_decode($this->dataMail[0]->bcc);
        $replyToData = json_decode($this->dataMail[0]->replyto);

        $content = json_decode($this->dataMail[0]->content, true);

        $subject = $this->dataMail[0]->subject;
        ...
        if( $this->dataMail[0]->estado_nombre == 'DECLINADO' ){
        ...
        */
        ...
        }elseif( $this->dataMail[0]->estado_nombre == 'ACEPTADO' ){
            $markdownWith = array_merge($markdownWith,[
