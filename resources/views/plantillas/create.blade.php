...
@section('head_vars')
  ...
  $pagetitle = "Crear Plantilla - ".$plantilla['nombre'];
  ...
  <div id="content_editor" class="container">
    <div id="content_columns" class="text-left">
      ...
      <div class="columns">
        ...
        <h2><label for="editor1">Editor de plantillas</label></h2>  

        <p>
          <h3>A continuacion se muestra el contenido correspondiente a <strong>{{ $plantilla['nombre'] }}</strong></h3>
          <br>
          @if(isset($imprimir))
            <?php $complemento = 'e <strong>imprimir</strong> <span class="cke_button__print_icon" style="width: 16px;display: inline-block;" >&nbsp;</span> la'; ?>
          @else 
            <?php $complemento = 'de la'; ?>
          @endif
          <h4>Puede usar las diferentes opciones de edicion para modificar el contenido {!! $complemento !!} plantilla.</h4>
          <br/>
          ...
          @endif
          @if(isset($editar))
            <h4>Cuando termine de realizar las modificaciones en la plantilla escoja la opción <strong>'Guardar'</strong> {!! $complemento !!}</h4>
            <br/>
        ...
        <div class="col-sm-12 editor_plantilla hide">
          <div class="document_editor editor">
            @if(isset($imprimir))
              <div class="editor_title_msg">
                <h4><i class="fa fa-hand-o-down"></i> Escoja esta opción para imprimir la plantilla</h4>
              </div>
            @endif
            <textarea id="editor1" col="10" row="6" class="hide">
              {!! ($plantilla_contenido ?? old(plantilla_contenido) ?? '') !!}
            </textarea>
          </div>
          <div class="contacts">
            <div>
              <h3>Puede arrastar a la plantilla algún elemento de esta lista</h3>
              <ul id="contactList">
                @if(isset($keyWords))
                  @php 
                    $data_contact = 1; 
...
@section('scripts')
  ...
  <script type="text/javascript">
    ...
    /*
      ...
      $('select[name="tipo_plantilla"]').on('change', function(){
        var thisVal = $(this).val();
        if (thisVal == { { $pre_forma_id }}) {
          $('.archivo_input').addClass('hide');
          $('.editor_plantilla').removeClass('hide');
        }else{
          $('.editor_plantilla').addClass('hide');
      ...
      */
      ...
      $('button#guardar_plantilla').on('click', function(){
        var form_target = $(this).attr('form_target');
        var data = CKEDITOR.instances.editor1.getData();

        $('form'+ form_target +' input[name="plantilla_contenido"]').val(data);
